...
export async function createLocalUser(input: {
  ...
  const role: "user" | "seller" | "admin" =
    isFirstUser || isOwnerEmail ? "admin" : (input.requestedRole ?? "user");
...
export async function syncFirebaseUser(input: {
  ...
  if (existing) {
    await touchLastSignedIn(existing.id);
    return getUserById(existing.id);
  }

  // Firebase owns the password. A random unusable local hash keeps the
  // existing schema compatible while preventing local-password login.
  return createLocalUser({
    name: input.name,
    email: input.email,
    passwordHash: `firebase:${randomUUID()}`,
  ...
  if (!db)
    return seedProducts.filter(
      p =>
        ...
        (!category || category === "الكل" || p.category === category)
    );
...
export async function getSettings() {
  ...
  return rows.reduce(
    (acc, row) => ({ ...acc, [row.settingKey]: row.settingValue }),
    defaultSettings
  );
}
...
export async function createStoreForOwner(input: {
  ownerId: number;
  name: string;
  slug: string;
  description?: string;
  ...
  const created = await db
    .select()
    .from(stores)
    .where(eq(stores.id, Number(result[0].insertId)))
    .limit(1);
  return created[0];
}
