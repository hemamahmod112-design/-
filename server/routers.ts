...
import { verifyFirebaseIdToken } from "./_core/firebaseToken";
import {
  createSessionToken,
  hashPassword,
  isValidEmail,
...
const firebaseSyncInput = z.object({
...
const settingsInput = z
  .object({
    currency: z.enum(["SAR", "AED", "USD", "EGP"]).optional(),
...
export const appRouter = router({
  ...
  auth: router({
    ...
    firebaseSync: publicProcedure
      .input(firebaseSyncInput)
      .mutation(async ({ input, ctx }) => {
        let identity;
        try {
          identity = await verifyFirebaseIdToken(input.idToken);
        } catch {
          throw new TRPCError({
            code: "UNAUTHORIZED",
            message: "تعذر التحقق من حساب Firebase",
          });
        }
        ...
        const user = await syncFirebaseUser({
          ...
          name:
            input.name?.trim() || identity.name || identity.email.split("@")[0],
          requestedRole: input.type,
        });
        if (!user)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "تعذر مزامنة الحساب",
          });
  ...
  customer: router({
    orders: protectedProcedure.query(({ ctx }) =>
      listOrdersForCustomerById(ctx.user.id)
    ),
  }),
  seller: router({
    ...
    createStore: sellerProcedure
      .input(storeInput)
      .mutation(async ({ input, ctx }) => {
        if (await getStoreByOwnerId(ctx.user.id))
          throw new TRPCError({
            code: "CONFLICT",
            message: "لديك متجر مسجل بالفعل",
          });
        try {
          return createStoreForOwner({ ...input, ownerId: ctx.user.id });
        } catch (error) {
    ...
    products: sellerProcedure.query(async ({ ctx }) => {
      const store = await getStoreByOwnerId(ctx.user.id);
      return store ? listProductsForStore(store.id) : [];
    }),
    createProduct: sellerProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return createProduct({ ...input, storeId: store.id });
      }),
    updateProduct: sellerProcedure
      .input(z.object({ id: z.number().int(), data: productInput.partial() }))
      .mutation(async ({ input, ctx }) => {
        const store = await getStoreByOwnerId(ctx.user.id);
        if (!store)
          throw new TRPCError({
        ...
        const result = await updateProductForStore(
          store.id,
          input.id,
          input.data
        );
    ...
    deleteProduct: sellerProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return deleteProductForStore(store.id, input.id);
      }),
  }),
  admin: router({
